...
class Keyboard:
    @classmethod
    def reply_keyboard(cls, buttons_text: Tuple, **kwargs):
        keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
        buttons = []
        for text in buttons_text:
            if kwargs:
                buttons.append(types.KeyboardButton(text=text, **kwargs))
                continue
...
class GeneralKeyboard(Keyboard):
    ...
    blog_url = 'https://t.me/mrhmarketing'
    ...
    def enter_menu(self, doc=False):
        ...
        key6 = types.InlineKeyboardButton(text='Блог', url=self.blog_url)
        keyboard.add(key1)
        if doc is True:
            keyboard.add(key3)
    ...
    def directions(self) -> types.InlineKeyboardMarkup:
        ...
        logger.info(list_of_directions)
...
class ClientKeyboard(Keyboard):
    ...
    def send_phone(self):
        buttons_text = ("Отправить номер телефона",)
        return self.reply_keyboard(buttons_text, request_contact=True)
    ...
    def evaluation(self):
        text_and_callbacks = {
            ...
            self.client_grade_no_button.text: self.client_grade_no_button.data,
            self.enter_menu_for_client_button.text: self.enter_menu_for_client_button.data}
...
class OperatorKeyboard(Keyboard):
    ...
    def client_files(self, dict_of_path_files, in_dialogue=None):
        ...
        if dict_of_path_files is None:
            keyboard.row(upload_file, cancel)
            return keyboard
        else:
