...
class Database:
    ...
    def get_user_data(self, user_id: int) -> Dict[str, Any]:
        ...
        Returns:
            ...
            | date_of_registration | str | В формате '%d-%m-%Y'. |
            | id | int | telegram_id |
            | name | str | Имя |
            | tg_username | str | Имя (telegram) |
            | phone | str | Номер телефона |
            | company | str | Компания |
            | website | str | Сайт |
            | documents | str | Наличие файлов или документов |
        """
        try:
    ...
    def get_users_data(self, user_ids: list) -> List[dict[str, Any]]:
